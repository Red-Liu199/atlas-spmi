...
class Atlas(nn.Module):
    def __init__(self, opt, reader, retriever, reader_tokenizer, retriever_tokenizer):
        ...
        self.opt = opt
        self.decoder_only = not ('t5' in opt.reader_model_type)
    ...
    def tokenize_casual(self, query, passages, target, add_title=True):
        # query passages
        training_passages = []
        test_passages = []
        for q, ps, t in zip(query, passages, target):
            ...
            for p in ps:
                if add_title:
                    context.append('title: {} context: {}'.format(p['title'], p['text']))
                else:
                    context.append(p['text'])
            context.append('answer: '+t)
            test_context.append('answer: ')
            training_passages.append(';'.join(context))
            test_passages.append(';'.join(test_context))
        ...
        test_tok = self.reader_tokenizer(
            test_passages,
            padding="max_length",
            max_length=self.opt.text_maxlength,
            return_tensors="pt",
            truncation=True,
        ) # FIXME: the padding must be performed from the left during generation.
        reader_tok = _to_cuda(reader_tok)
        test_tok = _to_cuda(test_tok)
        

        target_tok = reader_tok["input_ids"]
        attention_mask = reader_tok['attention_mask']
        input_tok = self.reader._shift_right(target_tok)
        labels = target_tok.masked_fill(~attention_mask.bool(), IGNORE_INDEX)
        return input_tok, attention_mask, labels, test_tok
    ...
    def forward(
        ...
        if self.decoder_only:
            assert self.opt.task=='vanilla_qa', "Only vanilla qa template can be used for decoder-only structure"
            assert not train_retriever, "Retriever cannot be trained in decoder-only structure currently"
            input_tok, attention_mask, labels, _ = self.tokenize_casual(query, passages, target)
            reader_output = self.reader(
                input_ids=input_tok,
