...
def load_reader(opt):
    ...
    if not opt.retrieve_only:
        if 't5' in opt.reader_model_type:
            ...
            # decoder-only reader
            reader = AutoModelForCausalLM.from_pretrained(opt.reader_model_path)
            reader_tokenizer = transformers.AutoTokenizer.from_pretrained(opt.reader_model_path)

    return reader, reader_tokenizer


def _set_reader_encoder_cfg(model, opt):
    if model.reader is not None and hasattr(model.reader, 'encoder'):
