...
def evaluate(model, index, opt, data_path, step=None):
    ...
    for i, batch in enumerate(data_iterator):
        ...
        if (len(query) == 0) or (len(query[0]) == 0):
            continue
        if unwrapped_model.decoder_only:
            input_tok, attention_mask, labels, generate_inputs = unwrapped_model.tokenize_casual(query, retrieved_passages, answers)
            ...
            if "eval_loss" in task.metrics:
                eval_loss, logits = reader_output[0].cpu().item(), reader_output[1]
            generation = unwrapped_model.reader.generate(
